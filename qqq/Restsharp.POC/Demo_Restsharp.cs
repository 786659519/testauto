...
namespace RestSharp_1
{
    ...
    public class Demo_Restsharp
    {
        ...
        ///
        private RestResponseCookie Login()
        {
            IRestHelper rest = new IRestHelper();
            var client= rest.AddUrl("http://app.360scm.com/SCM.Cloud.Web");
            // var client = new RestClient("http://testxa.360scm.com/SCM.Cloud.Web");              //基地址                                             fz
            ...
            //var requestPost = new RestRequest("/Login/Login", Method.POST);                       


            requestPost.AddHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");//添加http头信息                              fz
            //requestPost.AddParameter("login", "userid=autotest&password=1c5f4612b9c19b343c89e896298efb32&token=", ParameterType.RequestBody);//添加http请求数据
